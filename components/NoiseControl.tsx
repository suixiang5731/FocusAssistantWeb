...
export const NoiseControl: React.FC = () => {
  ...
  return (
    <div className="relative" ref={panelRef}>
      {/* Toggle Bar Button */}
      <div className="flex items-center gap-0.5 sm:gap-1 bg-white/90 hover:bg-white border border-slate-200/80 shadow-2xs rounded-2xl p-0.5 sm:p-1 transition-all">
        <button
          type="button"
          onClick={togglePlay}
          className={`flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 rounded-xl font-semibold text-xs transition-all active:scale-95 ${
            isPlaying 
        ...
        >
          {isPlaying ? (
            <>
              ...
              <span className="text-xs">白噪声</span>
              <span className="text-[10px] bg-white/20 px-1 py-0.5 rounded-full">{currentNoiseObj.icon}</span>
            </>
          ) : (
            <>
        ...
        <button
          ...
          className="p-1 sm:p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-xl transition-colors"
          title="展开白噪声设置"
        >
      ...
      {isOpenPanel && (
        <div className="absolute right-0 top-11 z-40 w-60 sm:w-64 max-w-[calc(100vw-2rem)] bg-white rounded-2xl border border-slate-200 shadow-xl p-3.5 space-y-3 animate-modal-enter">
